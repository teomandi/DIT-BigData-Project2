...
class KMeans(object):
    ...
    def __init__(self, data_path, k=4, threshold=0.6, chunk_size=4000, distance_f="d1", ratings_path=None):
        ...
        if self.distance_f == "d1":
            ...
            self.fit = self.fit_with_ratings
    ...
    def fit_with_new(self):
        ...
        for chunk in pd.read_csv(self.data_path, chunksize=self.chunk_size):
            ...
            if iteration == 0:
                ...
                self.discard = [Cluster(i, index, chunk[self.target][index]) for i, index in enumerate(random_clusters_ids)]
    ...
    def fit_with_tags(self):
        ...
        for chunk in pd.read_csv(self.data_path, chunksize=self.chunk_size):
            ...
            if iteration == 0:
                ...
                self.discard = [Cluster(i, index, chunk_data[index], is_list=True) for i, index in
        ...
        for remain in self.remaining:
            ...
            for disc in self.discard:
                if self.distance_f != "d3":
                    dist.append(get_jaccard(remain.clusteroid, disc.clusteroid, is_list=self.is_list))
                else:
                    dist.append(cosine_similarity(remain.clusteroid, disc.clusteroid)[0][0])
